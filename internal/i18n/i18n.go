...
var messages = map[string]map[string]string{
	English: {
		...
		"runtime.legacyConfig":                     "Existing configuration uses the legacy single-profile format. Re-run initialization to create the new profile-based config.",
		...
		"wizard.title":                             "CloudCanal CLI profile initialization (%s)",
	...
	Chinese: {
		...
		"runtime.legacyConfig":                     "现有配置仍是旧的单环境格式，请重新初始化并生成新的 profile 配置。",
		...
		"wizard.title":                             "CloudCanal CLI profile 初始化（%s）",
